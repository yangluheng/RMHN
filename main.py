...
# os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"  #使用CPU
#os.environ["CUDA_VISIBLE_DEVICES"] = "-1"

#设置随机种子，可以在不同机器上生成相同随机数
seed = 12306
np.random.seed(seed)
...
if __name__ == '__main__':
    ...
    print("开始时间：",time.strftime("%Y-%m-%d %H:%M:%S",time1))
    start = datetime.datetime.now()

    print("正在执行"+args.dataset)
    config = Config(args.dataset,args.lang)
    e1 = set(loadfile(config.e1, 1))
    e2 = set(loadfile(config.e2, 1))
    e = len(e1 | e2)            #e1、e2交集
    print(e)
    ILL = loadfile(config.ill, 2)   #对齐实体对  [(0, 10500), (1, 10501), (2, 10502), (3, 10503), (4, 10504)]
    illL = len(ILL) #15000
    np.random.shuffle(ILL)          #重新排序，返回一个随机序列
    ...
    print("结束时间：",time.strftime("%Y-%m-%d %H:%M:%S", time2))
